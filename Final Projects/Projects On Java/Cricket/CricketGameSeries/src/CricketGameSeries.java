...
public class CricketGameSeries {
    ...
    public String chooseTeam() throws IOException {
        ...
        System.out.println("Choose team out of the following:");
    ...
    public int chooseOver() throws IOException {
        ...
        System.out.print("Choose no. of overs (1, 3, 5, or 10): ");
        while (flag == 0) {
            try {
    ...
    public String toss() throws IOException {
        ...
        if (!tossArr.contains(choice.toLowerCase())) {
            ...
            if (choice.equalsIgnoreCase(toss_Result)) {
                ...
                rnd = rand.nextInt(bat_bowl.length);
    ...
    public int[] batting(int over, int target) throws IOException {
        ...
        CricketGameSeries ob1 = new CricketGameSeries();
        int arr[] = new int[2];
        int limit = 6, user = 0;
    ...
    public int[] bowling(int over, int target) throws IOException {
        ...
        System.out.println();
        CricketGameSeries ob2 = new CricketGameSeries();
        int wickets = 0, runs = 0;
        int[] invalid = {1, 4};
        for (int i = 1; i <= over; i++) {
            inner:
            for (int j = 1; j <= 6; j++) {
                ...
                if (user < 1 || user > limit) {
                    ...
                    if (invalid[r] == 1) {
                        System.out.println("Wide! 1 run added to the opponent's score.");
                        runs += 1;
                    } else {
                        System.out.println("Byes! 4 runs added to the opponent's score.");
                        runs += 4;
                    }
                    j--;
                    continue inner;
    ...
    public static void restart() throws IOException {
        System.out.println("Press 0 to exit or 1 to restart the game.");
    ...
    public static void main(String[] args) throws IOException {
        ...
        System.out.println("2.) A wicket falls from the batting side if the bowler and batsman both take out the same number.");
        System.out.println("3.) You and the computer both have 3 wickets in total.");
        System.out.println("3.) Please take care of cases (exactly the same case as provided to you in choices) else the program restarts.");
        System.out.println("_________________________________");

        CricketGameSeries ob = new CricketGameSeries();
        String error = "xxx";
        ...
        if (playersTeam.equals(error)) {
            ...
            if (opp.equals(error)) {
                ...
                if (playersTeam.equalsIgnoreCase(opp)) {
                    System.out.println("You can't choose the same team as the opponent.");
                    ob.restart();
                }

                System.out.print("Enter the number of matches you want to play (odd number only): ");
                int numMatches = Integer.parseInt(br.readLine());

                if (numMatches % 2 == 0) {
                    System.out.println("Number of matches should be odd. Program terminating.");
                    ob.restart();
                }
                ...
                for (int match = 1; match <= numMatches; match++) {
                    ...
                    if (over == -1) {
                        ...
                        System.out.println("************************ END OF MATCH " + match + " ************************");
                        System.out.println();
                    }
                }
                ...
                System.out.println(playersTeam + " Score: " + playerScore);
                System.out.println(opp + " Score: " + opponentScore);

                if (playerScore > opponentScore) {
                    System.out.println(playersTeam + " won the series!");
                } else if (opponentScore > playerScore) {
                    System.out.println(opp + " won the series!");
                } else {
